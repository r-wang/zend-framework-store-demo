...
(function(window, undefined) {
	...
	(function(window, undefined) {
		 ...
		 * 
		 * @param {Function}
		 *            fn The function to test for native support
		 */
		 ...
		 * @param {Function}
		 *            handler The method that will be applied if the test fails
		 * @param {Boolean}
		 *            test The result of a test. If true, null will be set as
		 *            the handler in leiu of the specified handler
		 */
		function addHandle(attrs, handler, test) {
			...
			while (i--) {
				...
				if (!(current = Expr.attrHandle[attrs[i]])
						|| current === handler) {
		...
		function boolHandler(elem, name) {
			...
			return val && val.specified ? val.value
		...
		setDocument = Sizzle.setDocument = function(node) {
			...
			support.attributes = assert(function(div) {
				...
				addHandle("type|href|height|width", interpolationHandler,
						div.firstChild.getAttribute("href") === "#");

				// Support: IE<9
				// Use getAttributeNode to fetch booleans when getAttribute lies
				addHandle(booleans, boolHandler,
			...
			addHandle("value", valueHandler, support.attributes
					&& support.input);
			...
			contains = isNative(docElem.contains)
			...
			// Support: Webkit<537.32 - Safari 6.0.3/Chrome 25 (fixed in Chrome
			// 27)
	...
	jQuery.support = (function(support) {
		...
		var all, a, input, select, fragment, opt, eventName, isSupported, i, div = document
				.createElement("div");
		...
		jQuery(function() {
			var container, marginDiv, tds, divReset = "padding:0;margin:0;border:0;display:block;box-sizing:content-box;-moz-box-sizing:content-box;-webkit-box-sizing:content-box;", body = document
	...
	function internalRemoveData(elem, name, pvt) {
		...
		if (name) {
			...
			if (thisCache) {
				...
				if (pvt ? !isEmptyDataObject(thisCache) : !jQuery
						...
						while ((name = attrNames[i++])) {
							...
							if (jQuery.expr.match.bool.test(name)) {
								...
								if (getSetInput && getSetAttribute
									...
									elem[propName] = false;
									// Support: IE<9
									// Also clear defaultChecked/defaultSelected
									// (if appropriate)
								} else {
	...
	jQuery.each(jQuery.expr.match.bool.source.match(/\w+/g), function(i, name) {
		...
		jQuery.expr.attrHandle[name] = getSetInput && getSetAttribute
				|| !ruseDefault.test(name) ? function(elem, name, isXML) {
			...
			return isXML ? undefined
					: elem[jQuery.camelCase("default-" + name)] ? name
							.toLowerCase() : null;
		};
	});
	...
	if (!getSetAttribute) {
		...
		nodeHook = {
			set : function(elem, value, name) {
				...
				if (!ret) {
					elem.setAttributeNode((ret = elem.ownerDocument
		...
		function(elem, name, isXML) {
			var ret;
			return isXML ? undefined : (ret = elem.getAttributeNode(name))
	...
	jQuery.event = {
		...
		dispatch : function(event) {
			...
			var i, ret, handleObj, matched, j, handlerQueue = [], args = core_slice
					.call(arguments), handlers = (jQuery._data(this, "events") || {})[event.type]
	...
	jQuery.fn
			.extend({
				...
				closest : function(selectors, context) {
					...
					return this.pushStack(ret.length > 1 ? jQuery.unique(ret)
							: ret);
				},
					...
					function(name, original) {
						jQuery.fn[name] = function(selector) {
							...
							for (; i <= last; i++) {
								...
								// Modern browsers can apply jQuery collections
								// as arrays, but oldIE needs a .get()
	...
	function getAll(context, tag) {
		var elems, elem, i = 0, found = typeof context.getElementsByTagName !== core_strundefined ? context
				.getElementsByTagName(tag || "*")
				: typeof context.querySelectorAll !== core_strundefined ? context
					...
					for (; i < l; i++) {
						...
						if (elem || elem === 0) {
							...
							if (jQuery.type(elem) === "object") {
								...
								// Remember the top-level container for proper
								// cleanup
								tmp = safe.lastChild;
							}
						}
			...
			if (show) {
				...
				if (!values[index]) {
					...
					if (display && display !== "none" || !hidden) {
						jQuery._data(elem, "olddisplay", hidden ? display
	...
	if (window.getComputedStyle) {
		...
		curCSS = function(elem, name, _computed) {
			var width, minWidth, maxWidth, computed = _computed
					|| getStyles(elem),
	...
	jQuery.fn
			.extend({
				...
				finish : function(type) {
					...
					return this
							.each(function() {
								var index, data = jQuery._data(this), queue = data[type
